...
describe("Comment it", () => {
  ...
  it('comment with login but post id not existing', async () => {
      ...
      await assertFails(col.doc('non-exisiting-post-id').set({ uid: 'uid', content: 'content' }));
  });
  ...
  it('update comment', async () => {
      const db = await setup(myAuth, mockData);
      const col = db.collection('posts').doc('post-id-1').collection('comments').doc('comment-id-1');
      await firebase.assertSucceeds(col.update({
          content: 'content change',
      }));
  });
